...
const canary_1 = require("../src/canary");
const canary = canary_1.CanaryTest.Group("Canary Test");
const allTests = [];
...
// And now, the tests...
addTest(async function testReferencesToCanaryClasses() {
    // Check CanaryTest class
    assert(canary instanceof canary_1.CanaryTest);
    // Check CanaryTestCallback class
    const callback = canary.onBegin(() => { });
    assert(callback instanceof canary_1.CanaryTest.Callback);
    // Check CanaryTestError class
    const error = canary.addError(new Error("!!"));
    assert(error instanceof canary_1.CanaryTest.Error);
});
addTest(async function testSimpleSynchronousTests() {
    // Set up the test
    ...
    assert(simpleFailingTest.getErrors()[0] instanceof canary_1.CanaryTest.Error);
    assert(simpleFailingTest.getErrors()[0].message === "Simple test failure");
    assert(canary.getStatusString() === "failed");
});
addTest(async function testSimpleAsynchronousTests() {
    // Set up the test
...
addTest(async function testWithPassingSyncAndAsyncTests() {
    // Set up the test
    ...
    let nestedSyncPassingTest = null;
    let nestedAsyncPassingTest = null;
...
addTest(async function testExecutionOrderOfTests() {
    // Set up the test
    let counter = 0;
...
addTest(async function testGroupVsTestSeriesBehavior() {
    // Set up the test
    let counter = 0;
...
addTest(async function testGettingNamesAndTitles() {
    // Set up the test
    let test = null;
    let callback = null;
...
addTest(async function testDoReportFilteringBehavior() {
    // Set up the test
...
addTest(async function testSuccessfulGroupCallbackOrder() {
    // Set up the test
    let counter = 0;
...
addTest(async function testSuccessfulGroupEachCallbackOrder() {
    // Set up the test
...
addTest(async function testAddingAndCheckingTags() {
...
addTest(async function testGroupFailedChildren() {
    // Set up the test
...
addTest(async function testTodoFlag() {
...
addTest(async function testIgnoreAndUnignore() {
    const someTest = canary.test("Example test", () => { });
...
addTest(async function testApplyFilterAndResetFilter() {
    // Set up tests
...
addTest(async function testParentsAndChildren() {
    // Set up the test
    let firstTest = null;
    let secondTest = null;
    const testGroup = canary.group("Example test group", function () {
    ...
    const isolatedTest = new canary_1.CanaryTest("Some test outside the test tree", () => { });
    // Run canary
    await canary.run();
...
addTest(async function testDuration() {
    // Set up the test
...
addTest(async function testLogFunction() {
...
addTest(async function testGetReport() {
    // Set up the test
    let firstTest = null;
    let secondTest = null;
    let thirdTest = null;
